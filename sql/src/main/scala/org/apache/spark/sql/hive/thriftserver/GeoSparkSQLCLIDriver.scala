...
import org.apache.spark.sql.hive.thriftserver.SparkSQLEnv.{init, sparkContext, sqlContext, stop}
...
/**
  ...
  */
private[hive] object GeoSparkSQLCLIDriver extends Logging {
  ...
  def main(args: Array[String]) {
    ...
    ShutdownHookManager.addShutdownHook { () => stop() }
    ...
    val cli = new GeoSparkSQLCLIDriver
    ...
    if (sessionState.database != null) {
      sqlContext.sessionState.catalog.setCurrentDatabase(
...
private[hive] class GeoSparkSQLCLIDriver extends CliDriver with Logging {
  private val sessionState = SessionState.get().asInstanceOf[CliSessionState]

  private val LOG = LogFactory.getLog(classOf[GeoSparkSQLCLIDriver])
  ...
  private val isRemoteMode = {
    GeoSparkSQLCLIDriver.isRemoteMode(sessionState)
  }
  ...
  override def setHiveVariables(hiveVariables: java.util.Map[String, String]): Unit = {
    hiveVariables.asScala.foreach(kv => sqlContext.conf.setConfString(kv._1, kv._2))
  }
